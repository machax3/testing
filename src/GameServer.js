...
class GameServer {
    ...
    // In GameServer.js - Update the resolveCollision method
    resolveCollision(m) {
        let cell = m.cell,
            check = m.check;
    ...
    // Ensure ejected mass has a delay before being consumed
    if (cell.cellType === 3) { // Ejected mass
        if (!cell.ejectTime) {
            cell.ejectTime = Date.now(); // Set timestamp if missing
        }

        let delayTime = 760; // 0.5 seconds delay
        let timeElapsed = Date.now() - cell.ejectTime;

        // Apply delay ONLY if the eater is the same as the ejector
        if (cell.ejector && cell.ejector === check.owner) { 
            if (timeElapsed < delayTime) {
                return; // Prevent self-consumption before delay
            }
        }
    }
        ...
        // Check if the eaten cell is a bullet (EjectedMass) and the eater is a bot
        if (cell.cellType === 3 && check.owner && check.owner.isBot) {
            // Notify the bot that it was fed a bullet by the player
            check.owner.onEatBullet(cell.ejector);
        }
