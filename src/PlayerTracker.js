...
class PlayerTracker {
    ...
    getTotalMass() {
        return this.cells.reduce((sum, cell) => sum + cell._mass, 0);
    }

    getLargest(cells) {
        return cells.reduce((max, cell) => cell._mass > (max?._mass || 0) ? cell : max, null);
    }
    getDistanceTo(position) {
        return Math.hypot(position.x - this.centerPos.x, position.y - this.centerPos.y);
    }
    // Add this method to PlayerTracker
    getLargest(list) {
    ...
    checkConnection() {
        if (this.socket.isConnected === false) {
            ...
            this.socket.packetHandler.pressSpace = this.socket.packetHandler.pressW = this.socket.packetHandler.pressW2 = this.socket.packetHandler.pressW3 = this.socket.packetHandler.pressW4 = this.socket.packetHandler.pressW5 = this.socket.packetHandler.pressW6 = this.socket.packetHandler.pressQ = false;
            return;
        }
  ...
  this.gameServer.ejectMass(this)
    ...
    pressW() {
    if (this.gameServer.running && !this.isSpectating) {
        let ejectedMass = this.gameServer.ejectMass(this);
        if (ejectedMass) {
            ejectedMass.ejectTime = Date.now();
            console.log(`Ejected mass created with timestamp: ${ejectedMass.ejectTime}`);
        }
    }
	this.gameServer.ejectMass(this)
	this.gameServer.ejectMass(this)
